...
- La caída de tensión del alimentador pilar-tablero genera hallazgo, no solo cálculo `[AUTO]`. Antes se calculaba y se mostraba en "Mediciones y cálculos", pero al no ser hallazgo no llegaba a la lista de acciones del informe ni la contaba el overview. Son tres casos: la caída medida supera el umbral configurado (`PRIORITY`), la estimada lo supera y no hay mediciones para confirmarla (`RECOMMENDED`, la acción es medir), y la medida es muy superior a la estimada (`PRIORITY`, apunta al recorrido y no a la sección del cable). Entran como sugerencias de regla pendientes de validación del técnico.
- Los umbrales configurables (`ElectricalRuleConfig`) llegan al armado de hallazgos, no solo a los cálculos `[AUTO]`. Antes `InspectionFindingProposalBuilder` leía siempre `DefaultElectricalRuleConfigs`, así que un umbral editado por el técnico podía dar "aceptable" en el cálculo y abrir un hallazgo en la misma pantalla. `InspectionFeederVoltageDrop` centraliza la lectura de mediciones y umbral para que ambos consumidores clasifiquen igual.
