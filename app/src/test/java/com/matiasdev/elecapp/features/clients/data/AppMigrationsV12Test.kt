...
class AppMigrationsV12Test {
    @Test
    fun `migration 11 to 12 adds visual inspection fields and section review statuses`() {
        val db = RecordingDatabaseV12()

        MIGRATION_11_12.migrate(db)

        assertTrue(db.sql.contains("ALTER TABLE electrical_inspections ADD COLUMN review_reason TEXT"))
        assertTrue(db.sql.contains("ALTER TABLE electrical_inspections ADD COLUMN reviewed_element TEXT"))
        assertTrue(db.sql.contains("ALTER TABLE electrical_inspections ADD COLUMN task_description TEXT"))
        assertTrue(db.sql.contains("ALTER TABLE pillar_inspections ADD COLUMN review_status TEXT NOT NULL DEFAULT 'REVIEWED'"))
        assertTrue(db.sql.contains("ALTER TABLE main_panel_inspections ADD COLUMN review_status TEXT NOT NULL DEFAULT 'REVIEWED'"))
    }
}

private class RecordingDatabaseV12 : SupportSQLiteDatabase {
