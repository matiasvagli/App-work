...
fun QuoteFormScreen(
    ...
    val uiState by viewModel.uiState.collectAsStateWithLifecycle()
    LaunchedEffect(uiState.savedQuoteId) { uiState.savedQuoteId?.let(onSaved) }
        ...
        LazyColumn(
            ...
            item {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    OutlinedButton(onClick = { viewModel.save(QuoteSaveMode.DRAFT) }) { Text("Guardar borrador") }
                    Button(onClick = { viewModel.save(QuoteSaveMode.READY) }) { Text("Marcar listo") }
                }
            }
        }
    }
...
fun QuoteDetailScreen(
    ...
    Scaffold(modifier = modifier, topBar = { SimpleTopBar("Detalle de presupuesto", onBackClick) }) { padding ->
